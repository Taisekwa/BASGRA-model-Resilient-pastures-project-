...
# Andrea added, check whether first entry in harvest file is -1 (requirement to qualify for automated harvest)
# If first entry is -1, then fill the days_harvest matrix with -1
if (days_harvest[1,1]==-1){
  days_harvest <- matrix( -1, nrow=100, ncol=3 )
}
...
# DMH <- ((output$CLV+output$CST+output$CSTUB)/0.45 + output$CRES/0.40 + output$CLVD/0.45) * 10.0 / output$BASAL
# plot(x, DMH)
# abline(h=28, col="blue")
# harvest <- filter(output, HARVFR > 0)
# harvest_time <- harvest$Time
# abline(v=harvest_time, col="blue")
# basal_half_life_pos <- Position(function(x) x < 50, output$BASAL) # returns index of first occurrence of basal <50 (in days since the beginning of the run)
# cat(file = stderr(), basal_half_life_pos, "\n")
